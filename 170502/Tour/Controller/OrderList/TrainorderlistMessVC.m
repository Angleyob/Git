...
#import "SPmencell.h"
@interface TrainorderlistMessVC ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation TrainorderlistMessVC
...
- (void)viewDidLoad {
    ...
    if (![_orderStr isEqualToString:@""]) {
        olabel.text=[NSString stringWithFormat:@"订单号:%@",_orderStr];
    }else{
        olabel.text=[NSString stringWithFormat:@"订单号:%@",@""];
    }
    [viewtitle addSubview:olabel];
    _Messagedict=_Alldata[@"trainTicket"];

    if(([_Messagedict[@"canSendApprove"] isEqualToString:@"0"] ||[_Messagedict[@"canSendApprove"] isEqualToString:@""] )&& [_Messagedict[@"canCancel"] isEqualToString:@"0"] &&[_Messagedict[@"canPay"] isEqualToString:@"1"]){
    ...
    }else if ([_Messagedict[@"canCancel"] isEqualToString:@"0"] && [_Messagedict[@"canPay"] isEqualToString:@"0"]){
        UIView * bottomview  = [[UIView alloc]initWithFrame:CGRectMake(0, deviceScreenHeight-60, deviceScreenWidth, 60)];
        bottomview.backgroundColor=[UIColor colorWithRed:246/255.0 green:246/255.0 blue:246/255.0 alpha:1];
        ...
        button2.layer.cornerRadius = 10.0;
        [bottomview addSubview:button2];
    }else if ([_Messagedict[@"canRefund"] isEqualToString:@"0"] && [_Messagedict[@"canEndorse"] isEqualToString:@"0"]){
        UIView * bottomview  = [[UIView alloc]initWithFrame:CGRectMake(0, deviceScreenHeight-60, deviceScreenWidth, 60)];
        bottomview.backgroundColor=[UIColor colorWithRed:246/255.0 green:246/255.0 blue:246/255.0 alpha:1];
        ...
        [button2 addTarget:self action:@selector(SPclick:) forControlEvents:UIControlEventTouchUpInside];
        [button2 setTitleColor:[UIColor colorWithRed:247/255.0 green:181/255.0 blue:42/255.0 alpha:1] forState:UIControlStateNormal];
        button2.tag=307;
        //剪切
        button2.clipsToBounds=YES;
        //圆角
        ...
        [bottomview addSubview:button2];
    }else if ([_Messagedict[@"orderStatus2"] isEqualToString:@"3"]){
...
//        NSLog(@"%@",_Alldata[@"trainTicket"]);[31]	(null)	@"" : @"常小俊"
...
-(void)SPclick:(UIButton*)send{
    if(send.tag==301||send.tag==303||send.tag==304){
        ...
        //取消订单
        TrainOrderCancel *  foc =[TrainOrderCancel new];
        ...
        [Train TrainOrderCancel:foc success:^(id data) {
             [SVProgressHUD dismiss];
...
//                [self dismissViewControllerAnimated:YES completion:nil];
 ...
 [self.navigationController  pushViewController:pvc animated:YES];
        ...
        //申请退票
        TrainTPorderMessVC * tpvc = [TrainTPorderMessVC new];
        tpvc.statusStr=_statusStr;
        tpvc.orderStr=_orderStr;
        tpvc.Alldata=_Alldata;
        [self presentViewController:tpvc animated:NO completion:nil];
    }else if(send.tag==307){
        //申请改签
        [UIAlertView showAlertWithTitle1:@"线上改签功能暂时无法使用，请拨打客服电话:☎️ 4008-566-966进行线下改签" duration:1.5];
