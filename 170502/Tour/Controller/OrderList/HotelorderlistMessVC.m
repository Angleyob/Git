...
@interface HotelorderlistMessVC ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation HotelorderlistMessVC
...
if ([_statusStr isEqualToString:@"已取消"]) {
        ...
        bottomview.backgroundColor=[UIColor colorWithRed:246/255.0 green:246/255.0 blue:246/255.0 alpha:1];
    ...
    if([_Messagedict[@"canCancel"] isEqualToString:@"1"] && [_Messagedict[@"canSendApprove"] isEqualToString:@"1"] && ![_Messagedict[@"canPay"] isEqualToString:@"1"]){
        ...
        button2.layer.cornerRadius = 10.0;
        [bottomview addSubview:button2];
    }else if ([_Messagedict[@"canCancel"] isEqualToString:@"1"] && [_Messagedict[@"canSendApprove"] isEqualToString:@"0"] && [_Messagedict[@"canPay"] isEqualToString:@"0"]){
        UIView * bottomview  = [[UIView alloc]initWithFrame:CGRectMake(0, deviceScreenHeight-60, deviceScreenWidth, 60)];
        [self.view addSubview:bottomview];
        ...
        [button1 setTitleColor:[UIColor colorWithRed:247/255.0 green:181/255.0 blue:42/255.0 alpha:1] forState:UIControlStateNormal];
        [bottomview addSubview:button1];
    }else if ([_Messagedict[@"canCancel"] isEqualToString:@"1"] && [_Messagedict[@"canSendApprove"] isEqualToString:@"0"] && [_Messagedict[@"canPay"] isEqualToString:@"1"]){
        UIView * bottomview  = [[UIView alloc]initWithFrame:CGRectMake(0, deviceScreenHeight-60, deviceScreenWidth, 60)];
        bottomview.backgroundColor=[UIColor colorWithRed:246/255.0 green:246/255.0 blue:246/255.0 alpha:1];
        [self.view addSubview:bottomview];
        UIButton * button1= [[UIButton alloc]initWithFrame:CGRectMake(15, 8, (deviceScreenWidth-45)/2, 44)];
...
-(void)loadDataF{
    float roomPrice = [_Messagedict[@"totalPrice"]  floatValue]/[_Messagedict[@"roomNight"]  floatValue];
              tprice.text=[NSString stringWithFormat:@"房间价格:￥%.1f元     ×%@间",roomPrice,_Messagedict[@"roomNum"]];
            free.text=[NSString stringWithFormat:@"入住天数:             %.0f晚", [_Messagedict[@"roomNight"] floatValue]/[_Messagedict[@"roomNum"] floatValue]];
...
-(void)creattableview{
    ...
    tprice.textAlignment=NSTextAlignmentLeft;
...
//            butimage.image=[UIImage imageNamed:@"chevron_hide"];
            ...
            //    if([_DandYStr isEqualToString:@"D"]){[46]	(null)	@"linkInfoList" : @"1 element"	[0]	(null)	@"contactName" : @"常小俊"	[1]	(null)	@"mobile" : @"18802036336"
...
-(void)SPclick:(UIButton*)send{
    if(send.tag==301||send.tag==303||send.tag==304){
        ...
        HotelOrderCancel *  foc =[HotelOrderCancel new];
        ...
        foc.OrderNo=_orderStr;
        foc.CancelReason=@"不想去";
        foc.OrderType=@"1";
        foc.Version=@"4";
        [Hotel HotelOrderCancel:foc success:^(id data) {
...
//                [self dismissViewControllerAnimated:YES completion:nil];
                ...
                NSRange range;
                NSString *string;
                while ((range = [data[@"message"] rangeOfString:@"<[^>]+>" options:NSRegularExpressionSearch]).location != NSNotFound){
                    data[@"message"]=[data[@"message"] stringByReplacingCharactersInRange:range withString:@""];
                }
                
//                NSLog(@"Un block string : %@",data[@"message"]);
                [UIAlertView showAlertWithTitle:[NSString stringWithFormat:@"%@☎️:4008566966",data[@"message"]]];
            }
        } failure:^(NSError *error) {
...
//        NSLog(@"%@",_Messagedict);
        ...
        pvc.BusinessType=@"17";
        pvc.Hprice=_Messagedict[@"totalPrice"];
...
//[self presentViewController:pvc animated:NO completion:nil];
        ...
        //重新申请
//        for (FirstViewController * vc in [UIApplication sharedApplication]) {
...
//        
         ...
         */
        //把通知的内容放入字典中
        NSDictionary *dict = @{@"num": @"3"};
        //自定义的通知
        [nc postNotificationName:@"num" object:self userInfo:dict];
