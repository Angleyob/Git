...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (model.style == CellDayTypeFutur || model.style == CellDayTypeWeek ||model.style == CellDayTypeClick) {
        ...
        if (model.style == CellDayTypeClick) {
            ...
            if ([_outOrIn isEqualToString:@"out"]||[_outOrback isEqualToString:@"back"]) {
                if (_flag > 1) {
                    _flag = 1;
                }
                self.flag--;
        ...
        if ([_type isEqualToString:@"hotel"]) {
            if ([_outOrIn isEqualToString:@"out"]) {
                NSDate * d1=  [LGLCalendarDate dateFromString:_indate];
                NSDate * d2=  [LGLCalendarDate dateFromString:[model toString]];
                NSInteger h=[weekday getDaysFrom:d1 To:d2];
                if (h<=0) {
                    model.style = CellDayTypeClick;

                    [UIAlertView showAlertWithTitle1:[NSString stringWithFormat:@"您选择的入住日期为：%@离店日期不能早于该日期,请重新选择",_indate] duration:1.2];
                    model.style = CellDayTypeFutur;
            ...
            if ([_outOrback isEqualToString:@"back"]){
                if (![_outdate isEqualToString:@""]) {
                    NSDate * d1=  [LGLCalendarDate dateFromString:_outdate];
                    NSDate * d2=  [LGLCalendarDate dateFromString:[model toString]];
                    NSInteger h=[weekday getDaysFrom:d1 To:d2];
                    if (h<0) {
                        model.style = CellDayTypeClick;
                        [UIAlertView showAlertWithTitle1:[NSString stringWithFormat:@"您选择的出发日期为：%@返回日期不能早于该日期,请重新选择",_outdate] duration:1.2];
                        model.style = CellDayTypeFutur;
